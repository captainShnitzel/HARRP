...
static void hal_hardwareReset(sh2_Hal_t *self);

static bool i2c_write(sh2_Hal_t *self, const uint8_t *buffer, size_t len, bool stop = true,
        const uint8_t *prefix_buffer = nullptr, size_t prefix_len = 0);

static bool i2c_read(sh2_Hal_t *self, uint8_t *buffer, size_t len, bool stop = true);
static bool _i2c_read(sh2_Hal_t *self, uint8_t *buffer, size_t len, bool stop);

static bool hal_wait_for_int(sh2_Hal_t *self);
...
static bool spi_read(sh2_Hal_t *self, uint8_t *buffer, size_t len, uint8_t sendvalue = 0xFF);
static bool spi_write(sh2_Hal_t *self, const uint8_t *buffer, size_t len,
			const uint8_t *prefix_buffer = nullptr, size_t prefix_len = 0);
...
static inline BNO08x *ownerFromHal(sh2_Hal_t *self)
{
    auto *ctx = reinterpret_cast<BNO08x::HalContext *>(self);
    return ctx->owner;
}
...
boolean BNO08x::begin(uint8_t deviceAddress, TwoWire &wirePort, int8_t user_INTPin, int8_t user_RSTPin)
{
	...
	if(user_RSTPin != -1)
	{
		_resetPin = user_RSTPin;
		pinMode(_resetPin, INPUT_PULLUP);
	}

  	if (_intPin != -1) hal_wait_for_int(&_halCtx.hal);
    ...
    _halCtx.owner = this;
    _halCtx.hal.open = i2chal_open;
    _halCtx.hal.close = i2chal_close;
    _halCtx.hal.read = i2chal_read;
    _halCtx.hal.write = i2chal_write;
    _halCtx.hal.getTimeUs = hal_getTimeUs;
...
boolean BNO08x::beginSPI(uint8_t user_CSPin, uint8_t user_INTPin, uint8_t user_RSTPin, uint32_t spiPortSpeed, SPIClass &spiPort)
{
	...
	digitalWrite(_csPin, HIGH); //Deselect BNO08x

	_spiPort->begin(); //Turn on SPI hardware

	_halCtx.owner = this;
	_halCtx.hal.open = spihal_open;
	_halCtx.hal.close = spihal_close;
	_halCtx.hal.read = spihal_read;
	_halCtx.hal.write = spihal_write;
	_halCtx.hal.getTimeUs = hal_getTimeUs;
...
float BNO08x::getQuatI()
{
	...
	// }
	// return (quat);
	return _sensorValuePtr->un.rotationVector.i;
}
...
float BNO08x::getQuatJ()
{
	...
	//return (quat);
	return _sensorValuePtr->un.rotationVector.j;
}
...
{
	return _sensorValuePtr->un.accelerometer.x;
}

//Return the acceleration component
float BNO08x::getAccelY()
{
	return _sensorValuePtr->un.accelerometer.y;
}

//Return the acceleration component
float BNO08x::getAccelZ()
{
	return _sensorValuePtr->un.accelerometer.z;
}
...
{
	return _sensorValuePtr->un.linearAcceleration.x;
}

//Return the acceleration component
float BNO08x::getLinAccelY()
{
	return _sensorValuePtr->un.linearAcceleration.y;
}

//Return the acceleration component
float BNO08x::getLinAccelZ()
{
	return _sensorValuePtr->un.linearAcceleration.z;
}
...
{
	return _sensorValuePtr->un.gyroscope.x;
}

//Return the gyro component
float BNO08x::getGyroY()
{
	return _sensorValuePtr->un.gyroscope.y;
}

//Return the gyro component
float BNO08x::getGyroZ()
{
	return _sensorValuePtr->un.gyroscope.z;
}
...
{
	x = _sensorValuePtr->un.gyroscopeUncal.x;
	y = _sensorValuePtr->un.gyroscopeUncal.y;
	z = _sensorValuePtr->un.gyroscopeUncal.z;
	bx = _sensorValuePtr->un.gyroscopeUncal.biasX;
	by = _sensorValuePtr->un.gyroscopeUncal.biasY;
	bz = _sensorValuePtr->un.gyroscopeUncal.biasZ;
	accuracy = _sensorValuePtr->status;
}
//Return the gyro component
float BNO08x::getUncalibratedGyroX()
{
	return _sensorValuePtr->un.gyroscopeUncal.x;
}
//Return the gyro component
float BNO08x::getUncalibratedGyroY()
{
	return _sensorValuePtr->un.gyroscopeUncal.y;
}
//Return the gyro component
float BNO08x::getUncalibratedGyroZ()
{
	return _sensorValuePtr->un.gyroscopeUncal.z;
}
//Return the gyro component
float BNO08x::getUncalibratedGyroBiasX()
{
	return _sensorValuePtr->un.gyroscopeUncal.biasX;
}
//Return the gyro component
float BNO08x::getUncalibratedGyroBiasY()
{
	return _sensorValuePtr->un.gyroscopeUncal.biasY;
}
//Return the gyro component
float BNO08x::getUncalibratedGyroBiasZ()
{
	return _sensorValuePtr->un.gyroscopeUncal.biasZ;
}
...
{
	return _sensorValuePtr->un.gravity.x;
}

//Return the gravity component
float BNO08x::getGravityY()
{
	return _sensorValuePtr->un.gravity.y;
}

//Return the gravity component
float BNO08x::getGravityZ()
{
	return _sensorValuePtr->un.gravity.z;
}
...
{
	return _sensorValuePtr->un.magneticField.x;
}

//Return the magnetometer component
float BNO08x::getMagY()
{
	return _sensorValuePtr->un.magneticField.y;
}

//Return the magnetometer component
float BNO08x::getMagZ()
{
	return _sensorValuePtr->un.magneticField.z;
}
...
{
	return _sensorValuePtr->un.stepCounter.steps;
}

//Return the stability classifier
uint8_t BNO08x::getStabilityClassifier()
{
	return _sensorValuePtr->un.stabilityClassifier.classification;
}

//Return the activity classifier
uint8_t BNO08x::getActivityClassifier()
{
	return _sensorValuePtr->un.personalActivityClassifier.mostLikelyState;
}

//Return the activity confindence
uint8_t BNO08x::getActivityConfidence(uint8_t activity)
{
	return _sensorValuePtr->un.personalActivityClassifier.confidence[activity];
}

//Return the time stamp
uint64_t BNO08x::getTimeStamp()
{
	return _sensorValuePtr->timestamp;
}

//Return raw mems value for the accel
int16_t BNO08x::getRawAccelX()
{
	return _sensorValuePtr->un.rawAccelerometer.x;
}
//Return raw mems value for the accel
int16_t BNO08x::getRawAccelY()
{
	return _sensorValuePtr->un.rawAccelerometer.y;
}
//Return raw mems value for the accel
int16_t BNO08x::getRawAccelZ()
{
	return _sensorValuePtr->un.rawAccelerometer.z;
}

//Return raw mems value for the gyro
int16_t BNO08x::getRawGyroX()
{
	return _sensorValuePtr->un.rawGyroscope.x;
}
int16_t BNO08x::getRawGyroY()
{
	return _sensorValuePtr->un.rawGyroscope.y;
}
int16_t BNO08x::getRawGyroZ()
{
	return _sensorValuePtr->un.rawGyroscope.z;
}

//Return raw mems value for the mag
int16_t BNO08x::getRawMagX()
{
	return _sensorValuePtr->un.rawMagnetometer.x;
}
int16_t BNO08x::getRawMagY()
{
	return _sensorValuePtr->un.rawMagnetometer.y;
}
int16_t BNO08x::getRawMagZ()
{
	return _sensorValuePtr->un.rawMagnetometer.z;
}
...
{
  int status = sh2_devReset_ctx(_sh2);
...
{
  int status = sh2_devSleep_ctx(_sh2);
...
{
  int status = sh2_setCalConfig_ctx(_sh2, sensors);
...
{
  int status = sh2_setTareNow_ctx(_sh2, zAxis ? TARE_AXIS_Z : TARE_AXIS_ALL, basis);
...
{
  int status = sh2_persistTare_ctx(_sh2);
...
{
  int status = sh2_clearTare_ctx(_sh2);
...
{
  int status = sh2_saveDcdNow_ctx(_sh2);
  if (status != SH2_OK) {
    return false;
  }
  return true;	
...
bool BNO08x::_init(int32_t sensor_id) {
  ...
  _sh2 = sh2_open_ctx(&_halCtx.hal, hal_callback, this);
  if (_sh2 == nullptr) {
    return false;
  }

  memset(&prodIds, 0, sizeof(prodIds));
  if (sh2_getProdIds_ctx(_sh2, &prodIds) != SH2_OK) {
    return false;
  }

  if (sh2_setSensorCallback_ctx(_sh2, sensorHandler, this) != SH2_OK) {
    return false;
  }
...
bool BNO08x::getSensorEvent() {
  _sensorValuePtr = &sensorValue;
  _sensorValuePtr->timestamp = 0;

  sh2_service_ctx(_sh2);

  if (_sensorValuePtr->timestamp == 0 &&
      _sensorValuePtr->sensorId != SH2_GYRO_INTEGRATED_RV) {
    return false;
  }
...
bool BNO08x::enableReport(sh2_SensorId_t sensorId, uint32_t interval_us,
  ...
  if (_intPin != -1) {
    if (!hal_wait_for_int(&_halCtx.hal)) {
      return false;
    }
  }

  int status = sh2_setSensorConfig_ctx(_sh2, sensorId, &config);
...
static int i2chal_open(sh2_Hal_t *self) {
  BNO08x *imu = ownerFromHal(self);
  if (imu->_intPin != -1) hal_wait_for_int(self);

  uint8_t softreset_pkt[] = {5, 0, 1, 0, 1};
  bool success = false;
  for (uint8_t attempts = 0; attempts < 5; attempts++) {
    if (i2c_write(self, softreset_pkt, 5)) {
      success = true;
      break;
    }
    delay(30);
...
(void)t_us;
  ...
  if (imu->_intPin != -1) {
	if (!hal_wait_for_int(self)) {
    	return 0;
  	}
  }

  uint8_t header[4];
  if (!i2c_read(self, header, 4)) {
    return 0;
  }
  ...
  */
  ...
  while (cargo_remaining > 0) {
    ...
    if (!i2c_read(self, i2c_buffer, read_size)) {
      return 0;
    }
...
static int i2chal_write(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len) {
  ...
  */
  ...
  uint16_t write_size = min(i2c_buffer_max, (size_t)len);
  ...
  if (!i2c_write(self, pBuffer, write_size)) {
    return 0;
  }
...
static void hal_hardwareReset(sh2_Hal_t *self) {
  BNO08x *imu = ownerFromHal(self);
  if (imu->_resetPin != -1) {
    pinMode(imu->_resetPin, OUTPUT);
    digitalWrite(imu->_resetPin, HIGH);
    delay(10);
    digitalWrite(imu->_resetPin, LOW);
    delay(10);
    digitalWrite(imu->_resetPin, HIGH);
    delay(10);
  }
...
static void hal_callback(void *cookie, sh2_AsyncEvent_t *pEvent) {
  BNO08x *imu = static_cast<BNO08x *>(cookie);
  if (imu != nullptr && pEvent->eventId == SH2_RESET) {
    imu->_resetOccurred = true;
  }
}
...
static void sensorHandler(void *cookie, sh2_SensorEvent_t *event) {
  ...
  sh2_SensorValue_t *value = imu->_sensorValuePtr ? imu->_sensorValuePtr : &imu->sensorValue;
 ...
 *
 */
void BNO08x::hardwareReset(void) { hal_hardwareReset(&_halCtx.hal); }


//Return the sensorID
uint8_t BNO08x::getSensorEventID()
{
	return _sensorValuePtr ? _sensorValuePtr->sensorId : sensorValue.sensorId;
}
 ...
 */
bool i2c_write(sh2_Hal_t *self, const uint8_t *buffer, size_t len, bool stop,
  ...
  imu->_i2cPort->beginTransmission(imu->_deviceAddress);

  // Write the prefix data (usually an address)
  if ((prefix_len != 0) && (prefix_buffer != nullptr)) {
    if (imu->_i2cPort->write(prefix_buffer, prefix_len) != prefix_len) {
      return false;
    }
  }

  // Write the data itself
  if (imu->_i2cPort->write(buffer, len) != len) {
    return false;
  }

  if (imu->_i2cPort->endTransmission(stop) == 0) {
    return true;
  } else {
    return false;
  }
 ...
 */
boolean i2c_read(sh2_Hal_t *self, uint8_t *buffer, size_t len, bool stop) {
  size_t pos = 0;
  while (pos < len) {
    size_t read_len =
        ((len - pos) > maxBufferSize()) ? maxBufferSize() : (len - pos);
    bool read_stop = (pos < (len - read_len)) ? false : stop;
    if (!_i2c_read(self, buffer + pos, read_len, read_stop))
      return false;
    pos += read_len;
  }
  return true;
...
boolean _i2c_read(sh2_Hal_t *self, uint8_t *buffer, size_t len, bool stop) {
  BNO08x *imu = ownerFromHal(self);
#if defined(TinyWireM_h)
  size_t recv = imu->_i2cPort->requestFrom((uint8_t)imu->_deviceAddress, (uint8_t)len);
#elif defined(ARDUINO_ARCH_MEGAAVR)
  size_t recv = imu->_i2cPort->requestFrom(imu->_deviceAddress, len, stop);
#else
  size_t recv = imu->_i2cPort->requestFrom((uint8_t)imu->_deviceAddress, (uint8_t)len, (uint8_t)stop);
#endif
...
static bool hal_wait_for_int(sh2_Hal_t *self) {
  BNO08x *imu = ownerFromHal(self);
  for (int i = 0; i < 500; i++) {
    if (!digitalRead(imu->_intPin))
      return true;
    delay(1);
  }
  hal_hardwareReset(self);
  return false;
}
...
static int spihal_read(sh2_Hal_t *self, uint8_t *pBuffer, unsigned len,
  ...
  if (!spi_read(self, pBuffer, 4, 0x00)) {
    return 0;
  }
  ...
  if (!spi_read(self, pBuffer, packet_size, 0x00)) {
    return 0;
  }
 ...
 * writes
 */
static bool spi_read(sh2_Hal_t *self, uint8_t *buffer, size_t len, uint8_t sendvalue) {
  BNO08x *imu = ownerFromHal(self);
  memset(buffer, sendvalue, len);

  imu->_spiPort->beginTransaction(SPISettings(imu->_spiPortSpeed, MSBFIRST, SPI_MODE3));
  digitalWrite(imu->_csPin, LOW);
  imu->_spiPort->transfer(buffer, len);
  digitalWrite(imu->_csPin, HIGH);
  imu->_spiPort->endTransaction();
  ...
  if (imu->_spiPort) {
    if (prefix_len > 0 && prefix_buffer != nullptr) {
      for (size_t i = 0; i < prefix_len; i++) {
        imu->_spiPort->transfer(prefix_buffer[i]);
      }
    }
    if (len > 0 && buffer != nullptr) {
      imu->_spiPort->transferBytes(const_cast<uint8_t *>(buffer), nullptr, len);
    }
  } else
#endif
  {
    ...
    for (size_t i = 0; i < len; i++) {
      imu->_spiPort->transfer(buffer[i]);
    }
  }

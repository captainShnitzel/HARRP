...
// SH2 state
sh2_t _sh2;
#define SH2_MAX_INSTANCES 8
static sh2_t g_sh2_instances[SH2_MAX_INSTANCES];
...
static sh2_t *allocSh2Instance(void)
{
    for (int i = 0; i < SH2_MAX_INSTANCES; i++) {
        if (g_sh2_instances[i].pHal == 0) {
            memset(&g_sh2_instances[i], 0, sizeof(sh2_t));
            return &g_sh2_instances[i];
        }
    }
    return 0;
}
...
/* Context-based multi-instance wrappers */
void sh2_close_ctx(void *ctx)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return;
    if (pSh2->pShtp) shtp_close(pSh2->pShtp);
    memset(pSh2, 0, sizeof(sh2_t));
}

int sh2_setSensorCallback_ctx(void *ctx, sh2_SensorCallback_t *callback, void *cookie)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return SH2_ERR_BAD_PARAM;
    pSh2->sensorCallback = callback;
    pSh2->sensorCookie = cookie;
    return SH2_OK;
}

int sh2_devReset_ctx(void *ctx)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return SH2_ERR_BAD_PARAM;
    return sendExecutable(pSh2, EXECUTABLE_DEVICE_CMD_RESET);
...
int sh2_getProdIds_ctx(void *ctx, sh2_ProductIds_t *prodIds)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2 || !prodIds) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
    pSh2->opData.getProdIds.pProdIds = prodIds;
    return opProcess(pSh2, &getProdIdOp);
}

int sh2_getSensorConfig_ctx(void *ctx, sh2_SensorId_t sensorId, sh2_SensorConfig_t *pConfig)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2 || !pConfig) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
    pSh2->opData.getSensorConfig.sensorId = sensorId;
    pSh2->opData.getSensorConfig.pConfig = pConfig;
    return opProcess(pSh2, &getSensorConfigOp);
...
int sh2_setSensorConfig_ctx(void *ctx, sh2_SensorId_t sensorId, const sh2_SensorConfig_t *pConfig)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2 || !pConfig) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
...
int sh2_setCalConfig_ctx(void *ctx, uint8_t sensors)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
...
int sh2_saveDcdNow_ctx(void *ctx)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
...
int sh2_setTareNow_ctx(void *ctx, uint8_t axes, sh2_TareBasis_t basis)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
...
int sh2_persistTare_ctx(void *ctx)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
...
int sh2_clearTare_ctx(void *ctx)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (!pSh2) return SH2_ERR_BAD_PARAM;
    memset(&pSh2->opData, 0, sizeof(sh2_OpData_t));
...
void *sh2_open_ctx(sh2_Hal_t *pHal,
                   sh2_EventCallback_t *eventCallback,
                   void *eventCookie)
{
    sh2_t *pSh2 = allocSh2Instance();
    if ((pSh2 == 0) || (pHal == 0)) return 0;
...
void sh2_service_ctx(void *ctx)
{
    sh2_t *pSh2 = (sh2_t *)ctx;
    if (pSh2 && pSh2->pShtp) shtp_service(pSh2->pShtp);
}
